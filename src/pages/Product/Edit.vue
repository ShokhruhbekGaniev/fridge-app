    ...
    export default {
        ...
        methods: {
            async submit() {
                ...
                if (confirm('Вы действительно хотите изменить данный продукт?')) {
                    await APIRequest(`http://localhost:3000/products/${this.$route.params.id}`, 'PUT', this.product);
                    this.redirectHome();
                }
            }
        },
        async mounted() {
            this.product = await APIRequest(`http://localhost:3000/products/${this.$route.params.id}`);
        }
    };
</script>